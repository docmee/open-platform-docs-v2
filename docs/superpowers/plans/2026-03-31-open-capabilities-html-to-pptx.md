...
**Goal:** 将 `开放能力 / HTML 转 PPTX` 从单页文档改造成 7 个子页面的多页结构，按“能力介绍、快速开始、❗限制条件、完整 API 文档、错误码说明、最佳实践、能力计费”组织内容。
...
- `content/open-capabilities/html-to-pptx/integration-notes.mdx` - ❗限制条件
...
**Modify:**
- `content/open-capabilities/_meta.tsx` - 把 `html-to-pptx` 定义成目录节点
...
```ts
const sectionMetaText = readFileSync('content/open-capabilities/_meta.tsx', 'utf8')
...
expect(readFileSync('content/open-capabilities/html-to-pptx/integration-notes.mdx', 'utf8')).toContain('1280 × 720')
expect(readFileSync('content/open-capabilities/html-to-pptx/integration-notes.mdx', 'utf8')).toContain('尽力缩放到目标尺寸')
...
expect(readFileSync('content/open-capabilities/html-to-pptx/api-reference.mdx', 'utf8')).toContain('/open-capabilities/html-to-pptx/integration-notes')
...
expect(readFileSync('content/open-capabilities/html-to-pptx/quickstart.mdx', 'utf8')).toContain('/open-capabilities/html-to-pptx/integration-notes')
expect(readFileSync('content/open-capabilities/html-to-pptx/best-practices.mdx', 'utf8')).toContain('/open-capabilities/html-to-pptx/integration-notes')
```
...
**Files:**
- Modify: `content/open-capabilities/_meta.tsx`
- Create: `content/open-capabilities/html-to-pptx/_meta.ts`

- [ ] **Step 1: Update the section meta to point to the nested directory**

Ensure `content/open-capabilities/_meta.tsx` still exposes `HTML 转 PPTX` as a navigation item, now backed by the `html-to-pptx/` directory.
...
- 鉴权方式
- `.slide` 根元素要求
- `.slide` 目标尺寸应为 `1280 × 720`
- 非 `1280 × 720` 时仍尝试转换并尽力缩放，但效果不保证
- 同一次请求中的所有 HTML 应尽量同一尺寸
- `htmls` 顺序规则
- `filename` 文件命名规则与默认行为
- `fileUrl` 为临时 OSS 地址
- 平台不长期保存
...
- `filename` stays optional and auto-appends `.pptx` when the suffix is omitted
...
- Verify: `content/open-capabilities/_meta.tsx`
...
Confirm:
- `❗限制条件` carries the prerequisite constraints
