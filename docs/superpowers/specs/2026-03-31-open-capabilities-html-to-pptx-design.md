...
建议在 `content/open-capabilities/_meta.tsx` 中将 `html-to-pptx` 作为目录节点展示为：
...
- 鉴权方式
- `.slide` 根元素要求
- `.slide` 元素目标尺寸应为 `1280 × 720`
- 非 `1280 × 720` 时系统仍会尽力缩放到目标尺寸，但效果不保证
- 同一次请求中的所有 HTML 应尽量同尺寸
- `htmls` 顺序即最终 PPT 页顺序
- `filename` 文件命名约定
...
| `filename` | `string` | 否 | 指定最终下载文件名；不传时使用系统默认文件名，不带 `.pptx` 后缀时服务端自动补全。 |
...
- `.slide` 根元素要求、`1280 × 720` 目标尺寸约束、临时 OSS 地址、平台不长期保存：放在 `❗限制条件`
- `filename` 的命名规则与默认行为：放在 `❗限制条件`，并在 `快速开始` 放最小示例
...
- 更新 `content/open-capabilities/_meta.tsx`
...
- `❗限制条件` 中明确出现：
  - `.slide` 根元素要求
  - `1280 × 720` 目标尺寸要求
  - 非目标尺寸时仍会尝试转换并尽力缩放，但效果不保证
  - 临时 OSS 下载地址说明
  - 平台不长期保存说明
- `快速开始`、`完整 API 文档`、`最佳实践` 中对参数说明均链接到 `❗限制条件`
- `完整 API 文档` 中明确出现：
  - 接口地址
  - 请求参数
  - 成功/部分成功/整体失败响应
